...
- `<media>.orig.srt`：Whisper 转录出的原始语言字幕
...
└─────────────────┘     └─────────────────────┘     └─────────────────┘
         │                         │
         ▼                         ▼
  subtitle_pipeline.py      Whisper (转录)
...
| 文件 | 作用 |
|------|------|
| `app/models/subtitle_pipeline.py` | **核心引擎**。包含 `SubtitlePipeline`（转录+翻译）和 `SubtitleJobManager`（Web 端的任务管理） |
...
调用 `_translate_segments()`，将转录出的文本分批送入 LLM：
...
Web UI 需要异步执行耗时任务（转录+翻译），`SubtitleJobManager` 提供了基于线程的任务调度：
...
两阶段工作流：
1. **先转录**: `process(stop_after_transcription=True)` 只生成 `.orig.srt`
...
- **Python**: >= 3.10
- **转录**: OpenAI Whisper（`faster-whisper` 或 `whisper.cpp`）

# media_subtitler 技术文档

> 本文档面向希望理解、修改或维护本代码库的开发者。

## 1. 项目概述

`media_subtitler` 是一个本地运行的字幕生成与翻译管道。输入是用户已有的视频文件，输出是字幕文件：
...
| 文件 | 作用 |
|------|------|
| `media_subtitler/` | 可 pip 安装的库封装。`pipeline.py` 通过 `importlib` 动态加载核心模块，避免强制依赖 Flask |

...
生成的 ASS 同时写入 `LayoutResX/Y`，明确使用 ARIB 显示画布比例；避免 1440×1080 等非方形像素 TS 将中日文字形再次横向拉宽。日文默认不加背景：将 FFmpeg 导出的默认黑底设为 ASS alpha `FF`（完全透明），替代此前的半透明黑底。字幕事件中显式指定的背景颜色和透明度仍优先，其他样式、原始字幕缓存与事件保持不变。

日文正文的默认字号按 mpv 内嵌字幕的默认大小换算：720 行画布使用 38，
所以 540 行 ARIB 画布使用 28.5，而不是 FFmpeg 导出的 36。这样可避免外挂
日文比内嵌字幕大约大 26%。只调整解码器的默认样式，显式字号、注音、颜色、
坐标和时间保持不变。此换算对应 mpv 默认设置，不跟随自定义的播放器字号。

中文默认字号在 540 行画布上为 24（此前为 16），其他画布按比例换算。
译文紧跟日文实际显示高度排列；双行字幕保留间隔，长句或空间不足时自动缩小，
不覆盖下一行日文。纯译文叠加文件使用相同布局。

检测到内嵌 `arib_caption` 时，程序使用 FFmpeg 的 **libaribcaption** 解码器
提取带定位信息的 ASS，而不是先丢弃布局转成 SRT。生成的 `.bilingual.ass`
保留日文的坐标、颜色、显式字号及时间，在每个日文正文行下面添加较小的中文译文。
...
合并 ASS 的显示效果可能与内嵌字幕不同。输出会修正上述画布比例并去掉默认黑底，但 ASS 并不等同于原始广播渲染；
